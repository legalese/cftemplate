  ...
  var processor = plaintemplate(
    ...
    function handler (token, context, stringify, callback) {
      ...
      // `(( if payingInCash begin )) conditional text (( end ))`
        ...
        var expressionText = directive.substring(3)
        ...
        if (context.hasOwnProperty(key)) {
          ...
          try {
            var expression = parseBooleanExpression(expressionText)
            var value = evalParsedBooleanExpression(expression, context)
            if (value) { stringify(token.content, context, handler, callback) } else { callback(null, '') }
          } catch (e) {
      ...
      // `(( unless payingInCash begin )) conditional text (( end ))`
        ...
        expressionText = directive.substring(7)
        ...
        if (context.hasOwnProperty(key)) {
          ...
          try {
            expression = parseBooleanExpression(expressionText)
...
function parseBooleanExpression (text) {
  ...
  var char
  for (char of text) {
    if (char === '(') { sexp.push([]) } else if (char === ')') {
      if (word) {
...
function evalParsedBooleanExpression (lst, context) {
  ...
  if (lst.length === 1) {
    if (typeof lst[0] === 'string') { return !!context[lst[0]] } else {
